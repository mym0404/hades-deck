...
pnpm cli prepare-llm --input out/ranked-candidates.csv --output out/anki-llm-input.csv --prompt-output out/anki-batch-prompt.txt
pnpm cli validate-anki --input out/anki-final.csv
```
...
- 최종 카드 내용은 `prepare-llm`가 생성한 프롬프트를 LLM에 넘겨 만든 뒤 `validate-anki`로 검사합니다.
